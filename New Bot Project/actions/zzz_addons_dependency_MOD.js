...
try {
    ...
    if(AddOns.settings.fixYouTubeLivestreams != true && AddOns.fixYouTubeLivestreams != false) {
...
AddOns.scan = function() {
    ...
    if(dir) {
        var file = true;
        try {
            fs.readFileSync('./actions/mod_info_MOD.js', 'utf8');
        } catch(e) {
            if(e) file = false;
        }
        if(file) {
            this.settings.highlightInfos == true && typeof(process.send) != 'function' ? console.log('\x1b[1m\x1b[4m\x1b[31mDBM Add-Ons\x1b[0m: Detected "\x1b[1mmod_info_MOD.js\x1b[0m" in your actions directory!\nIf you have problems while starting your bot, please delete this file.') : console.log('DBM Add-Ons: Detected "mod_info_MOD.js" in your actions directory! If you have problems while starting your bot, please delete this file.');
        }
    }
    return true;
}
    ...
    return new Promise((resolve, reject) => {
        ...
        if(forceInstallation == true && forceInstallationCheck == true) {
            ...
            for(var timeout = 0; timeout < 3; timeout++) {
                ...
                this.settings.highlightInfos == true && typeof(process.send) != 'function' ? console.log(`\x1b[1m\x1b[4m\x1b[32mDBM Add-Ons\x1b[0m: Update${timeout > 0 ? ` (Attempt: ${timeout+1})` : ``} of "\x1b[1m${nodeModule}\x1b[0m" is starting, please wait...`) : console.log(`DBM Add-Ons: Update${timeout > 0 ? ` (Attempt: ${timeout+1})` : ``} of "${nodeModule}" is starting, please wait...`);
                try {
            ...
            if(result == true) {
                ...
                this.settings.highlightInfos == true && typeof(process.send) != 'function' ? console.log(`\x1b[1m\x1b[4m\x1b[32mDBM Add-Ons\x1b[0m: Update of "\x1b[1m${nodeModule}\x1b[0m" is finished!`) : console.log(`DBM Add-Ons: Update of "${nodeModule}" is finished!`);
                resolve(nodeModule);
            } else {
                this.settings.highlightInfos == true && typeof(process.send) != 'function' ? console.log(`\x1b[1m\x1b[4m\x1b[31mDBM Add-Ons\x1b[0m: Update of "\x1b[1m${nodeModule}\x1b[0m" failed! Please run "\x1b[1mnpm install ${nodeModule} --save\x1b[0m" manually.`) : console.log(`DBM Add-Ons: Update of "${nodeModule}" failed! Please run "npm install ${nodeModule} --save" manually.`);
                reject(nodeModule);
            }

        } else {
            //Start installation only if needed / possible
            if(typeof(process.send) == 'function') {
                this.settings.highlightInfos == true && typeof(process.send) != 'function' ? console.log('\x1b[1m\x1b[4m\x1b[31mDBM Add-Ons\x1b[0m: New Node-Modules cannot be installed directly via DBM!\nPlease run your bot via Command Prompt.') : console.log('DBM Add-Ons: New Node-Modules cannot be installed directly via DBM!\nPlease run your bot via Command Prompt.');
                reject(nodeModule);
            }
            for(var timeout = 0; timeout < 3; timeout++) {
                var check = checkModule();
                ...
                const child_process = require('child_process');
                var command = 'npm install ' + nodeModule + " --save";
                this.settings.highlightInfos == true && typeof(process.send) != 'function' ? console.log(`\x1b[1m\x1b[4m\x1b[32mDBM Add-Ons\x1b[0m: Installation${timeout > 0 ? ` (Attempt: ${timeout+1})` : ``} of "\x1b[1m${nodeModule}\x1b[0m" is starting, please wait...`) : console.log(`DBM Add-Ons: Installation${timeout > 0 ? ` (Attempt: ${timeout+1})` : ``} of "${nodeModule}" is starting, please wait...`);
                try {
                    child_process.execSync(command, {cwd: process.cwd(), stdio:[0,1,2]});
                } catch(e) {}
            }
            ...
            if(result == true) {
                this.nodeModules.set(`${nodeModule}`, require(nodeModule));
                if(timeout !== 0) {
                    this.settings.highlightInfos == true && typeof(process.send) != 'function' ? console.log(`\x1b[1m\x1b[4m\x1b[32mDBM Add-Ons\x1b[0m: Installation of "\x1b[1m${nodeModule}\x1b[0m" is finished!`) : console.log(`DBM Add-Ons: Installation of "${nodeModule}" is finished!`);
                }
                resolve(nodeModule);
            } else {
                this.settings.highlightInfos == true && typeof(process.send) != 'function' ? console.log(`\x1b[1m\x1b[4m\x1b[31mDBM Add-Ons\x1b[0m: Installation of "\x1b[1m${nodeModule}\x1b[0m" failed! Please run "\x1b[1mnpm install ${nodeModule} --save\x1b[0m" manually.`) : console.log(`DBM Add-Ons: Installation of "${nodeModule}" failed! Please run "npm install ${nodeModule} --save" manually.`);
                reject(nodeModule);
            }
        }
    });
...
module.exports = {
    ...
    subtitle: function(data) {
        return `${data.forceInstallation == 'true' ? 'Force ' : ''} Install "${data.module}"`;
    },
    ...
    mod: function(DBM) {
        ...
        if(AddOns.settings.loadAddOns == true) {
            ...
            if(AddOns.settings.secureLoad !== true) {
                ...
                //BotDisconnect
                DBM.Bot.onDisconnect = function() {
                    //! BETA FEATURE
                    console.log(`${this.bot.user.username} disconnected!`);
                }
            }
            ...
            DBM.Bot.loadTags = function() {
                try {
                    ...
                    AddOns.settings.highlightInfos == true && typeof(process.send) != 'function' ? console.log('\x1b[1m\x1b[4m\x1b[32mDBM Add-Ons\x1b[0m: Prefixes loaded!') : console.log('DBM Add-Ons: Prefixes loaded!');
                } catch(e) {
                    DBM.Bot.tags = {};
                }
            }
            ...
            //Restart Bot
            DBM.Files.restartBot = function(Bot, filename) {
                //! BETA FEATURE
                const platform = require('os').platform;
                if(platform == 'win32' || platform == 'win64') {
                    if(typeof(process.send) == 'function') {
                        console.log('"DBM.Files.restart Bot" is currently only available within a Command Prompt.');
                        return false;
                    }
                    if(!Bot || !Bot.bot) {
                        return false;
                    } else if(Bot.bot instanceof DBM.DiscordJS.Client) {
                        var username = Bot.bot.user.username;
                        console.log(`Restarting ${username}!`);
                        Bot.bot.destroy().then(() => {
                            const child_process = require('child_process');
                            child_process.execSync(`start cmd.exe @cmd /k "node ${typeof(filename) == 'string' ? filename : 'bot.js'}"`, {cwd: process.cwd(), stdio:[0,1,2]});
                    ...
                    console.log('"DBM.Files.restartBot" is currently only avaible for Windows users!');
                    return false;
                }
            }
            ...
            DBM.Files.reloadBot = function(Bot) {
                ...
                if(!Bot || !Bot.bot) {
                    ...
                    Bot.$evts = {}; // Events
                    Bot.reformatData();
                    DBM.Files.readData(function() {
                        var $evts = Bot.$evts;
                        for(let i = 0; i < DBM.Events.data.length; i++) {
                            const d = DBM.Events.data[i];
                            if(d.length > 0 && $evts[String(i)]) {
                                Bot.bot.on(d[0], DBM.Events.callEvents.bind(DBM.Events, String(i), d[1], d[2], d[3], !!d[4], d[5]));
                            }
                        }
                    });
                    return true;
            ...
            if(AddOns.settings.fixYouTubeLivestreams == true) {
                AddOns.settings.highlightInfos == true && typeof(process.send) != 'function' ? console.log('\x1b[1m\x1b[4m\x1b[32mDBM Add-Ons\x1b[0m: YouTube Livestream Debug loaded!') : console.log('DBM Add-Ons: YouTube Livestream Debug loaded!');
            }
            ...
            AddOns.settings.highlightInfos == true && typeof(process.send) != 'function' ? console.log('\x1b[1m\x1b[4m\x1b[32mDBM Add-Ons\x1b[0m: Mods loaded!') : console.log('DBM Add-Ons: Mods loaded!');
        }
    }
}
//---------------------------------------------------------------