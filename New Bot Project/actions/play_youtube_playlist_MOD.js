...
short_description: "Play videos of a YouTube playlist as audio.",
...
action: function(cache) {
	...
	const ytapi = AddOns.require('simple-youtube-api');
	const apikey = this.evalMessage(data.apikey, cache);
	const playlist = this.evalMessage(data.url, cache);
	const options = {};

	//Check Input
	if(playlist === undefined || playlist === "" || playlist.length.length < 70) {
		return console.log('Please insert a valid playlist url!');
	}
